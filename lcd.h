...
{
	PORTA=cmd;
	PORTB&=~(1<<0);//makes RS pin low
	PORTB&=~(1<<1);//makes RW pin low
	PORTB|=(1<<2);//makes EN pin high to send a pulse
	_delay_ms(10);
	PORTB&=~(1<<2);//makes EN pin low to end the pulse
	_delay_ms(10);
}
void lcd_data(char data)
{
	PORTA=data;
	PORTB|=(1<<0);//makes RS pin high
	PORTB&=~(1<<1);//makes RW pin low
	PORTB|=(1<<2);//makes EN pin high to send a pulse
	_delay_ms(10);
	PORTB&=~(1<<2);//makes EN pin low to end the pulse
